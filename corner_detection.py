...
from skimage.feature import canny, corner_fast
...
image = imread('leaf-images/Quercus stellata/Quercus stellata (11).jpg', as_grey=True)
edges = canny(image, 2)
thresh = threshold_otsu(image)
binary = -(image > thresh)
binary = binary_erosion(binary_closing(binary_dilation(binary_dilation(binary))))
binary_edges = binary_dilation(canny(binary, 0.1))
veins = remove_small_objects((edges-np.logical_and(edges,binary_edges)), 3, connectivity=1, in_place=True)
